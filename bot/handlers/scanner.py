...
async def scan_proxies(message: types.Message):
    ...
    try:
        ...
        await edit_or_send(user_id, message, "🚀 Scanning...", cancel_kb())
        ...
        logger.exception("scan_proxies failed for user %s", user_id)
        await edit_or_send(user_id, message, "⚠️ Scan failed", cancel_kb())
    finally:
        reset_state(user_id)
